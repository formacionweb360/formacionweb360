...
export default function AdminPage({ user }) {
  ...
  const usuariosFiltrados = useMemo(() => {
    return filtroGrupo === "todos"
      ? usuarios
      : usuarios.filter(u => u.grupo_nombre === filtroGrupo);
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6">
      ...
      <div className="max-w-7xl mx-auto mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="bg-white px-6 py-3 rounded-lg shadow-sm text-sm">
          <span className="font-semibold text-indigo-600">{usuariosFiltrados.length}</span>{" "}
          usuarios •{" "}
          <span className="font-semibold text-green-600">{totalActivos}</span>{" "}
          activos
        </div>
      </div>
              ...
              {OPCIONES_ASISTENCIA.map(estado => {
                ...
                return (
                  <div key={estado} className="flex justify-between items-center">
                    ...
                    <span className={`text-xs text-white px-3 py-1 rounded-full ${config.color}`}>
                      {count}
                    </span>
                  </div>
                );
